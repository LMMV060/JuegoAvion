    ...
    public class VerticalScrollDynamic : MonoBehaviour
    {
        ...
        // actual sprite index
        // sprite central donde se encuentra el avión
        private int _spriteIndex = 0;

        // track if the player was in upper half on last frame
        // Utilizado para saber si el avión está en sprite inferior o superior
        private bool _wasPlayerInUpperHalf = false;

        // previous signed distance from player to tile border
        // Distancia que me separa hasta los bordes de tiles
        private float _prevPlayerToBorder;

        private void Awake()
        {
            // Recuperar referencia SpriteRenderer
        ...
        private void Start()
        {
            ...
            if (player == null)
            {
                player = GameManager.Instance.Player.GetComponent<PlayerDynamics>();
            }
        ...
        private void UpdateAdjacentTile()
        {
            ...
            // Comprobar en que mitad está el player
            bool isPlayerInUpperHalf = player.transform.position.y >= _tileA.bounds.center.y;

            // Buscar cual es el siguiente sprite a posicionar
            ...
            // Asignar sprite al adyacente
            _tileB.sprite = sprites[adjacentIndex];

            // position tile B by aligning bounds edges to avoid gaps/overlaps
            float tileA_top = _tileA.bounds.max.y;              //Calcular limite superior
            float tileA_bottom = _tileA.bounds.min.y;          //Calcular limite inferior
            float tileB_halfHeight = _tileB.bounds.extents.y; //Calcular mitad
            ...
            // Ver a que distancia del borde me encuentro
            _prevPlayerToBorder = GetPlayerToTileBorderDelta();
        }
